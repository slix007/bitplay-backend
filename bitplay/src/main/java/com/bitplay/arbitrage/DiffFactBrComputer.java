...
//                checkByDeltaPlan(delta_plan, br_close.get(i));
    ...
    DiffFactBr compute() throws ToWarningLogException {
        ...
        if (pos_mode == PosMode.RIGHT_MODE && pos_bo >= 0 && pos_ao > 0 && abs(pos_ao) > abs(pos_bo)) {
            wamBr = comp_wam_br_for_ok_open_long();
        } else if (pos_mode == PosMode.RIGHT_MODE && pos_bo <= 0 && pos_ao < 0 && abs(pos_ao) > abs(pos_bo)) {
            wamBr = comp_wam_br_for_ok_open_short();
        } else if (pos_mode == PosMode.RIGHT_MODE && pos_bo > 0 && abs(pos_ao) < abs(pos_bo)) {
            wamBr = comp_wam_br_for_ok_close_long();
        } else if (pos_mode == PosMode.RIGHT_MODE && pos_bo < 0 && abs(pos_ao) < abs(pos_bo)) {
            wamBr = comp_wam_br_for_ok_close_short();
        } else if (pos_mode == PosMode.LEFT_MODE && pos_bo >= 0 && pos_ao > 0 && abs(pos_ao) > abs(pos_bo)) {
            wamBr = comp_wam_br_for_b_open_long();
        } else if (pos_mode == PosMode.LEFT_MODE && pos_bo <= 0 && pos_ao < 0 && abs(pos_ao) > abs(pos_bo)) {
            wamBr = comp_wam_br_for_b_open_short();
        } else if (pos_mode == PosMode.LEFT_MODE && pos_bo > 0 && abs(pos_ao) < abs(pos_bo)) {
