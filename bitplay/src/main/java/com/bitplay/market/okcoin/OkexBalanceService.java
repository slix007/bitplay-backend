...
public class OkexBalanceService implements BalanceService {
    ...
    private FullBalance recalcEquity(AccountBalance account, Pos pObj, OrderBook orderBook, ContractType contractType) {
        ...
        if (eMark != null
            ...
            if (pObj.getPositionLong().signum() < 0) {
                ...
                if (entryPrice != null && entryPrice.signum() != 0) {
                    ...
                    final BigDecimal pos = (pObj.getPositionLong().negate()).multiply(pos_cm);
        ...
        return new FullBalance(new AccountBalance(
                wallet,
                available,
                eMark != null ? eMark.setScale(8, RoundingMode.HALF_UP) : null,
                eLast != null ? eLast.setScale(8, RoundingMode.HALF_UP) : null,
                eBest != null ? eBest.setScale(8, RoundingMode.HALF_UP) : null,
                eAvg != null ? eAvg.setScale(8, RoundingMode.HALF_UP) : null,
                margin,
                account.getUpl(),
                account.getRpl(),
                account.getRiskRate()
